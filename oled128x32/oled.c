...
{     
    ...
    switch (type) {
		...
		oled_set_pos(x, y);
		for(i=0; i<6; i++)
			oled_write_byte(OLED_DATA, F6x8[(u8)c][i], 1);
		break;
		...
		oled_set_pos(x, y);
		for(i=0; i<16; i++)
			oled_write_byte(OLED_DATA, F16X32[c*64+i*4], 1);
		
		oled_set_pos(x, y+1);
		for(i=0; i<16; i++)
			oled_write_byte(OLED_DATA, F16X32[c*64+i*4+1], 1);
		
		oled_set_pos(x, y+2);
		for(i=0; i<16; i++)
			oled_write_byte(OLED_DATA, F16X32[c*64+i*4+2], 1);
		
		oled_set_pos(x, y+3);
		for(i=0; i<16; i++)
			oled_write_byte(OLED_DATA, F16X32[c*64+i*4+3], 1);
    		break;
	...
	switch (type){
		case 1 :
			while(chr[j] != '\0'){
				...
				for(i = 0; i < 6;i++)
					oled_write_byte(OLED_DATA,F6x8[c][i],1);
...
static ssize_t oled_write(struct file *filp, const char __user *buf, size_t cnt, loff_t *off)
{
    ...
    printk("dis_format.type = %d \r\n", dis_format.type);
    printk("dis_format.buf = %s \r\n", dis_format.buf);
    oled_showstring(dis_format.x, dis_format.y, dis_format.buf,dis_format.type);
    return 0;
}
